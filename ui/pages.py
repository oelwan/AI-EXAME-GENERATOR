            ...
            if question_type == "coding":
                ...
                with st.expander(f"💻 Coding Question {i+1}"):
                    ...
                    if test_results:
                        ...
                        # Filter to make sure we're only showing test results that match this question
                        for test in test_results:
                            # Validate the test is appropriate for the current test cases
                            # Extract function name from test case to match with the current question
                            test_str = test.get('test', '')
                            
                            # If we have test cases with assertions for this question, validate the test belongs here
                            if test_cases and 'assert' in test_cases and 'assert' in test_str:
                                test_parts = test_str.split('assert ')
                                if len(test_parts) > 1:
                                    # Extract function name from test case
                                    func_match = re.search(r'(\w+)\(', test_parts[1])
                                    if func_match:
                                        test_func = func_match.group(1)
                                        # If function name in test case doesn't appear in this question's test cases, skip it
                                        if test_func not in test_cases:
                                            continue
                            
                            # Add this test to the results for this question
